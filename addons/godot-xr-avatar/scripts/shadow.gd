...
#set all nodes 
onready var arvrorigin = get_node(arvrorigin_path)
onready var arvrcamera = get_node(arvrcamera_path)
onready var left_controller = get_node(left_controller_path)
onready var right_controller = get_node(right_controller_path)
...
func _physics_process(delta) -> void:
	...
	if avatar_height_set == false:
		avatar_height = $Armature/Skeleton/character_height.transform.origin.y
		$Armature.scale *= get_parent().get_node("ARVRCamera").transform.origin.y / avatar_height
		armature_scale = $Armature.scale
		max_height = get_parent().get_node("ARVRCamera").transform.origin.y
		avatar_height_set = true
	...
	skeleton.transform.origin.y = get_parent().get_parent().get_current_player_height() - avatar_height + height_offset
	...
	var move = get_parent().get_parent().get_player_body_velocity()
